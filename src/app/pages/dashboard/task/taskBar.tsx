...
const TaskBar: FC<Props> = (props) => {
  ...
  const {
    ...
    const CountContactList =  async () => {
      ...
      const CountContactResponse = await getCounts(userId, 0, 4, body)
    ...
    const overallFilter = async (data:any) => {
      ...
      let CountContactResponse = await getCounts(data?.user, data?.team, 4, body)
    ...
    const rangeSelect = async (val:any) => {
      ...
      setMonth('');
      if(val == 7) {
        document.getElementById('taskBarCustomDateRange')?.click();
      } else { 
        setContactBar(val);
        setLoading(true);
      var body = {
      ...
      let CountContactResponse = await getCounts(reqId, teamId, 4, body)
    ...
    const userFilter = async (name:any, id:any) => {
      ...
      let CountContactResponse = await getCounts(id, 0, 4, body)
    ...
    const handleTeam = async (id:any, name:any) => {
      ...
      let CountContactResponse = await getCounts(0, id, 4, body)
    ...
    return(
        ...
        <div className="card mx-sm-1 mx-xl-2 bs_1 h-100 br_15 bar_chart">
            <div className="card-heade border-0 d-flex align-items-center justify-content-between px-5 pt-5">
                ...
                <button type='button' data-bs-toggle='modal' data-bs-target='#taskBarCustomSelect' className='d-none' id='taskBarCustomDateRange'>{intl.formatMessage({id: 'custom_date'})}</button>
                <div className='modal fade' id='taskBarCustomSelect' aria-hidden='true'>
