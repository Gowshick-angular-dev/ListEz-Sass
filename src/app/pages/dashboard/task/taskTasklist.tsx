...
const TaskTaskList: FC<Props> = (props) => {
  const intl = useIntl();
  const {
    data, users, teams
  } = props
  ...
  const CountContactList =  async () => {  
    ...
    var body = {
      ...
      const CountContactResponse = await getTasksCount(4, reqId, teamId, body)
  ...
  const rangeSelect = async (val:any) => {
    ...
    setToDate('');
    if(val == 7) {
      document.getElementById('taskTaskListCustomDateRange')?.click();
    } else { 
    setLoading(true);
  ...
  const userFilter = async (name:any, id:any) => {
    ...
    const CountContactResponse = await getTasksCount(4, id, 0, body)
    setCountContact(CountContactResponse.output); 
    setLoading(false);   
  }
...
const overallFilter = async (data:any) => {
  setLoading(true);
  const CountContactResponse = await getTasksCount(4, data?.user, data?.team, data)
  setCountContact(CountContactResponse.output);
  setLoading(false);
}
  ...
  const handleTeam = async (id:any, name:any) => {
    ...
    var body = {
      ...
      const CountContactResponse = await getTasksCount(4, 0, id, body)
      setCountContact(CountContactResponse.output);
      setLoading(false);
  }
        ...
        <div className="card mx-sm-1 mx-xl-2 bs_1 h-100 br_15 bar_chart">
            <div className="card-heade border-0 d-flex align-items-center justify-content-between px-5 pt-5">
              ...
              {list == 1 ? <>
                ...
                <button type='button' data-bs-toggle='modal' data-bs-target='#taskTaskListCustomSelect' className='d-none' id='taskTaskListCustomDateRange'>{intl.formatMessage({id: 'custom_date'})}</button>
                <div className='modal fade' id='taskTaskListCustomSelect' aria-hidden='true'>
