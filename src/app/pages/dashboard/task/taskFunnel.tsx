...
const TaskFunnel: FC<Props> = (props) => {
  const {
    data, teams, users
  } = props
  const intl = useIntl();
  ...
  const CountContactList =  async () => {  
    ...
    const body = {
      ...
      const CountContactResponse = await getMastersCount('task_status', 4, userId, teamId, body)
  ...
  const rangeSelect = async (val:any) => {
    ...
    setToDate('');
    if(val == 7) {
      document.getElementById('taskFunnelCustomDateRange')?.click();
    } else { 
      setLoading(true);
    var body = {
      'start_date': fromDate,
    ...
    const CountContactResponse = await getMastersCount('task_status', 4, reqId, teamId, body)
    setCountContact(CountContactResponse.output);  
    setLoading(false);    
  }
  }
  ...
  const overallFilter = async (data:any) => {
    setLoading(true);
    const CountContactResponse = await getMastersCount('task_status', 4, data?.user, data?.team, data)
    setCountContact(CountContactResponse.output);
    setLoading(false);
  }
  ...
  const userFilter = async (name:any, id:any) => {
    ...
    const CountContactResponse = await getMastersCount('task_status', 4, id, 0, body)
    setCountContact(CountContactResponse.output); 
    setLoading(false);   
  }
  ...
  const handleTeam = async (id:any, name:any) => {
    ...
    var body = {
      ...
      const CountContactResponse = await getMastersCount('task_status', 4, 0, id, body)
      setCountContact(CountContactResponse.output);
      setLoading(false);
  }
        ...
        <div className="card mx-sm-1 bs_1 br_15 h-100 mb-4">
          <div className="card-heade border-0 d-flex align-items-center justify-content-between flex-wrap px-5 pt-5">
              ...
              {list == 1 ? <>
                ...
                <button type='button' data-bs-toggle='modal' data-bs-target='#taskFunnelCustomSelect' className='d-none' id='taskFunnelCustomDateRange'>{intl.formatMessage({id: 'custom_date'})}</button>
                <div className='modal fade' id='taskFunnelCustomSelect' aria-hidden='true'>
