...
const LeadDoughnutOverview: FC<Props> = (props) => {
  const intl = useIntl();
  const {
    data, users, teams
  } = props
    ...
    const rangeSelectPie = (val:any) => {
      if(val == 7) {
        document.getElementById('leadPieCustomDateRangeOverview')?.click();
      } else {
        setContactPie(val);
        var body = {
        'start_date': fromDate,
    ...
    return(
        <>
        <div className='modal fade' id='leadPieChartCustomDateOverview' data-bs-backdrop="static" data-bs-keyboard="false" aria-hidden='true'>
        ...
        <div className="card mx-sm-1 mx-xl-2 bs_1 h-100 br_15">        
            <div className="card-heade border-0 d-flex align-items-center justify-content-end pt-5 px-5">        
              ...
              {list == 1 ? <>
                ...
                <button type='button' data-bs-toggle='modal' data-bs-target='#leadPieChartCustomDateOverview' className='d-none' id='leadPieCustomDateRangeOverview'>{intl.formatMessage({id: 'custom_date'})}</button>                
                </div>                
            </div>             
