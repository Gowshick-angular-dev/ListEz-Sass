...
const OverviewCount2Bar: FC<Props> = (props) => {
  const intl = useIntl();
  const {
    data, users, teams
  } = props
    ...
    const CountContactList =  async () => {
      ...
      const CountContactResponse = await getCounts(0, teamId, 2, body)
    ...
    const overallFilter = async (data:any) => {
      ...
      let CountContactResponse = await getCounts(data?.user, data?.team, 2, body)
    ...
    const rangeSelect = async (val:any) => {
      ...
      setMonth('');
      if(val == 7) {
        document.getElementById('leadBarCustomDateRangeOverview')?.click();
      } else { 
        setContactBar(val);
        setLoading(true);
      var body = {
      ...
      let CountContactResponse = await getCounts(reqId, teamId, 2, body)
    ...
    const userFilter = async (name:any, id:any) => {
      ...
      let CountContactResponse = await getCounts(id, 0, 2, body)
    ...
    const handleTeam = async (id:any, name:any) => {
      ...
      let CountContactResponse = await getCounts(0, id, 2, body)
    ...
    return(
        ...
        <div className="card mx-sm-1 mx-xl-2 bs_1 h-100 br_15 bar_chart">
            <div className="card-heade border-0 d-flex align-items-center justify-content-between px-5 pt-5">
                ...
                <span className='card-label fw-bolder fs-3 mb-1'>{intl.formatMessage({id: 'leads'})}</span>
                </h3>
                ...
                <button type='button' data-bs-toggle='modal' data-bs-target='#leadBarCustomSelectOverview' className='d-none' id='leadBarCustomDateRangeOverview'>{intl.formatMessage({id: 'custom_date'})}</button>
                <div className='modal fade' id='leadBarCustomSelectOverview' aria-hidden='true'>
