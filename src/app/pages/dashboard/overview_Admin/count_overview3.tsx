...
const OverviewCount3Bar: FC<Props> = (props) => {
  const intl = useIntl();
  const {
    data, users, teams
  } = props
    ...
    const CountContactList =  async () => {
      ...
      const CountContactResponse = await getCounts(0, teamId, 5, body)
    ...
    const overallFilter = async (data:any) => {
      ...
      let CountContactResponse = await getCounts(data?.user, data?.team, 5, body)
    ...
    const rangeSelect = async (val:any) => {
      ...
      setMonth('');
      if(val == 7) {
        document.getElementById('transactionBarCustomDateRangeOverview')?.click();
      } else { 
        setContactBar(val);
        setLoading(true);
      var body = {
      ...
      let CountContactResponse = await getCounts(reqId, teamId, 5, body)
    ...
    const userFilter = async (name:any, id:any) => {
      ...
      let CountContactResponse = await getCounts(id, 0, 5, body)
    ...
    const handleTeam = async (id:any, name:any) => {
      ...
      let CountContactResponse = await getCounts(0, id, 5, body)
    ...
    return(
        ...
        <div className="card mx-sm-1 mx-xl-2 bs_1 h-100 br_15 bar_chart">
            <div className="card-heade border-0 d-flex align-items-center justify-content-between px-5 pt-5">
                ...
                <span className='card-label fw-bolder fs-3 mb-1'>{intl.formatMessage({id: 'transaction'})}</span>
                </h3>
                ...
                <button type='button' data-bs-toggle='modal' data-bs-target='#transactionBarCustomSelectOverview' className='d-none' id='transactionBarCustomDateRangeOverview'>{intl.formatMessage({id: 'custom_date'})}</button>
                <div className='modal fade' id='transactionBarCustomSelectOverview' aria-hidden='true'>
