...
const LeadBar: FC<Props> = (props) => {
  ...
  const {
    ...
    const CountContactList =  async () => {
      ...
      const CountContactResponse = await getCounts(userId, 0, 2, body)
    ...
    const rangeSelect = async (val:any) => {
      ...
      setMonth('');
      if(val == 7) {
        document.getElementById('leadBarCustomDateRange')?.click();
      } else { 
        setContactBar(val);
        setLoading(true);
      var body = {
    ...
    return(
        ...
        <div className="card mx-sm-1 mx-xl-2 bs_1 h-100 br_15 bar_chart">
            <div className="card-heade border-0 d-flex align-items-center justify-content-between px-5 pt-5">
                ...
                <button type='button' data-bs-toggle='modal' data-bs-target='#leadBarCustomSelect' className='d-none' id='leadBarCustomDateRange'>{intl.formatMessage({id: 'custom_date'})}</button>
                <div className='modal fade' id='leadBarCustomSelect' aria-hidden='true'>
