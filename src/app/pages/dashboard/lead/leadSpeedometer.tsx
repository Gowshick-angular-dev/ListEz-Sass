...
const LeadSpeedometer: FC<Props> = (props) => {
  const intl = useIntl();
  const {
    data, users, teams
  } = props
    ...
    const CountSourceList =  async (reqId:any, team:any, body:any) => {
      setLoading(true);         
      const CountSourceResponse = await getGoalsCount(2, reqId, team, body)
    ...
    const rangeSelectPie = (val:any) => {
      if(val == 7) {
        document.getElementById('leadspeedometerCustomDateRange')?.click();
      } else {
      setContactPie(val);
      var body = {
        'start_date': fromDate,
    ...
    return(
        ...
        <div className='modal fade' id='leadspeedometerCustomDate' aria-hidden='true'>
            ...
            <div className="card-heade border-0 d-flex align-items-center justify-content-end pt-5 px-5">
              ...
              {list == 1 ? <>
                ...
                <button type='button' data-bs-toggle='modal' data-bs-target='#leadspeedometerCustomDate' className='d-none' id='leadspeedometerCustomDateRange'>{intl.formatMessage({id: 'custom_date'})}</button>                
                </div>                
            </div>
