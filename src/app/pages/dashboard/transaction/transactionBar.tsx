...
const TransactionBar: FC<Props> = (props) => {
  const intl = useIntl();
  const {
    data, users, teams
  } = props
    ...
    const CountContactList =  async () => {
      ...
      const CountContactResponse = await getCounts(userId, 0, 5, body)
    ...
    return(
        ...
        <div className="card mx-sm-1 mx-xl-2 bs_1 h-100 br_15 bar_chart">
            <div className="card-heade border-0 d-flex align-items-center justify-content-between px-5 pt-5">
                ...
                <button type='button' data-bs-toggle='modal' data-bs-target='#transactionBarCustomSelect' className='d-none' id='transactionBarCustomDateRange'>{intl.formatMessage({id: 'custom_date'})}</button>
