...
const TransactionDoughnut2: FC<Props> = (props) => {
  const intl = useIntl();
  const {
    data, users, teams
  } = props
    ...
    const rangeSelectPie = (val:any) => {
      if(val == 7) {
        document.getElementById('transactionPieCustomDateRange2')?.click();
      } else {
        setContactPie(val);
        var body = {
        'start_date': fromDate,
    ...
    return(
        <>
        <div className='modal fade' id='transactionPieChartCustomDate2' data-bs-backdrop="static" data-bs-keyboard="false" aria-hidden='true'>
            ...
            <div className="card-heade border-0 d-flex align-items-center justify-content-end pt-5 px-5">        
                ...
                <ul className="dropdown-menu db_dw_menu" aria-labelledby="selectDropdown">
                    ...
                    <li className="dropdown-item cursor-pointer" onClick={() => CountSourceList(reqId, {
                        ...
                        'end_date': toDate,
                        'filter': contactPie,
                      }, teamId, 'property_status')}>{intl.formatMessage({id: 'transaction_status'})}</li>
                    <li className="dropdown-item cursor-pointer" onClick={() => CountSourceList(reqId, {
                        'start_date': fromDate,
                        'end_date': toDate,
                        'filter': contactPie,
                    ...
                    {/* <li className="dropdown-item cursor-pointer" onClick={() => CountSourceList(reqId, {
                        ...
                        'end_date': toDate,
                        'filter': contactPie,
                      }, teamId, 'developer')}>{intl.formatMessage({id: 'builder'})}</li>   */}
                </ul>
                {roleId != 4 && <>
              ...
              {list == 1 ? <>
                ...
                <button type='button' data-bs-toggle='modal' data-bs-target='#transactionPieChartCustomDate2' className='d-none' id='transactionPieCustomDateRange2'>{intl.formatMessage({id: 'custom_date'})}</button>                
                </div>                
            </div>             
