...
const PropertyFunnel: FC<Props> = (props) => {
  const {
    data, teams, users
  } = props
  const intl = useIntl();
  ...
  const CountContactList =  async () => {  
    ...
    const body = {
      ...
      const CountContactResponse = await getMastersCount('property_status', 3, userId, teamId, body)
  ...
  const rangeSelect = async (val:any) => {
    ...
    const CountContactResponse = await getMastersCount('property_status', 3, reqId, teamId, body)
    setCountContact(CountContactResponse.output);  
    setLoading(false);    
  }
  }
  ...
  const overallFilter = async (data:any) => {
    setLoading(true);
    const CountContactResponse = await getMastersCount('property_status', 3, data?.user, data?.team, data)
    setCountContact(CountContactResponse.output);
    setLoading(false);
  }
  ...
  const userFilter = async (name:any, id:any) => {
    ...
    const CountContactResponse = await getMastersCount('property_status', 3, id, 0, body)
    setCountContact(CountContactResponse.output); 
    setLoading(false);   
  }
  ...
  const handleTeam = async (id:any, name:any) => {
    ...
    var body = {
      ...
      const CountContactResponse = await getMastersCount('property_status', 3, 0, id, body)
      setCountContact(CountContactResponse.output);
      setLoading(false);
  }
    ...
    return(
        ...
        <div className="card-heade border-0 d-flex align-items-center justify-content-between flex-wrap px-5 pt-5">
              ...
              {list == 1 ? <>
                ...
                <button type='button' data-bs-toggle='modal' data-bs-target='#propertyFunnelCustomSelect' className='d-none' id='propertyFunnelCustomDateRange'>{intl.formatMessage({id: 'custom_date'})}</button>
