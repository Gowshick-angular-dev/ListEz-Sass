...
const ContactBar: FC<Props> = (props) => {
  const intl = useIntl();
  const {
    data, users, teams
  } = props
    ...
    console.log("lfhgerkjeterjthi", roleId);
    ...
    const rangeSelect = async (val:any) => {
      ...
      setMonth('');
      if(val == 7) {
        document.getElementById('contactBarCustomDateRange')?.click();
      } else { 
        setContactBar(val);
        setLoading(true);
      var body = {
    ...
    return(
        ...
        <div className="card mx-sm-1 mx-xl-2 bs_1 h-100 br_15 bar_chart">
            <div className="card-header border-0 d-flex align-items-center justify-content-between px-5 pt-5">
                ...
                <button type='button' data-bs-toggle='modal' data-bs-target='#contactBarCustomSelect' className='d-none' id='contactBarCustomDateRange'>{intl.formatMessage({id: 'custom_date'})}</button>
                <div className='modal fade' id='contactBarCustomSelect' aria-hidden='true'>
