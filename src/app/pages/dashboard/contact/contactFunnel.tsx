...
const ContactFunnel: FC<Props> = (props) => {
  const {
    data, teams, users
  } = props
  const intl = useIntl();
  ...
  const CountContactList =  async () => {  
    ...
    const body = {
      ...
      const CountContactResponse = await getMastersCount('contact_status', 1, userId, teamId, body)
  ...
  const rangeSelect = async (val:any) => {
    ...
    setToDate('');
    if(val == 7) {
      document.getElementById('contactFunnelCustomDateRange')?.click();
    } else { 
      setLoading(true);
    var body = {
      'start_date': fromDate,
    ...
    const CountContactResponse = await getMastersCount('contact_status', 1, reqId, teamId, body)
    setCountContact(CountContactResponse.output);  
    setLoading(false);    
  }
  }
  ...
  const overallFilter = async (data:any) => {
    setLoading(true);
    const CountContactResponse = await getMastersCount('contact_status', 1, data?.user, data?.team, data)
    setCountContact(CountContactResponse.output);
    setLoading(false);
  }
  ...
  const userFilter = async (name:any, id:any) => {
    ...
    const CountContactResponse = await getMastersCount('contact_status', 1, id, 0, body)
    setCountContact(CountContactResponse.output); 
    setLoading(false);   
  }
      ...
      onSubmit: async (values, {setStatus, setSubmitting, resetForm}) => {
        ...
        try { 
          ...
          if(CountContactResponse != null){
            ...
            resetForm();
            setLoading(false)
            document.getElementById('contactFunnelModelClose')?.click();
          }
  ...
  const handleTeam = async (id:any, name:any) => {
    ...
    var body = {
      ...
      const CountContactResponse = await getMastersCount('contact_status', 1, 0, id, body)
      setCountContact(CountContactResponse.output);
      setLoading(false);
  }
    ...
    return(
        ...
        <div className="card-heade border-0 d-flex align-items-center justify-content-between flex-wrap px-5 pt-5">
              ...
              {list == 1 ? <>
                ...
                <button type='button' data-bs-toggle='modal' data-bs-target='#contactFunnelCustomSelect' className='d-none' id='contactFunnelCustomDateRange'>{intl.formatMessage({id: 'custom_date'})}</button>
                <div className='modal fade' id='contactFunnelCustomSelect' aria-hidden='true'>
