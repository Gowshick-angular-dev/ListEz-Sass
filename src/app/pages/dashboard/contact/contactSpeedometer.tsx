...
import { getGoalsCount } from '../core/requests';
...
const ContactSpeedometer: FC<Props> = (props) => {
  const intl = useIntl();
  const {
    data, users, teams
  } = props
    ...
    const CountSourceList =  async (reqId:any, team:any, body:any) => {
      setLoading(true);         
      const CountSourceResponse = await getGoalsCount(1, reqId, team, body)
      setCount(CountSourceResponse.output[0]); 
    ...
    const rangeSelectPie = (val:any) => {
      if(val == 7) {
        document.getElementById('contactspeedometerCustomDateRange')?.click();
      } else {
      setContactPie(val);
      var body = {
        'start_date': fromDate,
      ...
      CountSourceList(reqId, teamId, body)
      }
    }
    ...
    useEffect(() => {
      if(data?.user != undefined) {
        ...
        CountSourceList(data?.user, data?.team, data)   
      }
    }, [data]);
    ...
    const handleTeam = async (id:any, name:any) => {
      ...
      CountSourceList(0, id, body)
    } 

    return(
        <>
        <div className="card mx-sm-1 mx-xl-2 h-100 bs_1 br_15">
        <div className='modal fade' id='contactspeedometerCustomDate' aria-hidden='true'>
            ...
            <div className="card-heade border-0 d-flex align-items-center justify-content-end pt-5 px-5">
                ...
                <ul className="dropdown-menu db_dw_menu" aria-labelledby="selectDropdown">
                    <li className="dropdown-item" onClick={() => CountSourceList(reqId, teamId, {
                        'start_date': fromDate,
                        'end_date': toDate,
                        'filter': contactPie,
                      })}>{intl.formatMessage({id: 'status_changed'})}</li>
                </ul>
                {roleId != 4 && <>
              ...
              {list == 1 ? <>
                ...
                <button type='button' data-bs-toggle='modal' data-bs-target='#contactspeedometerCustomDate' className='d-none' id='contactspeedometerCustomDateRange'>{intl.formatMessage({id: 'custom_date'})}</button>                
                </div>                
            </div>
              ...
              <div className="w-100 m-0">
                <div className="d-flex justify-content-center">
                    <ReactSpeedometer
                        ...
                        maxValue={100}
                        value={parseFloat((count.Achieved/count.goal_status_changed*100).toFixed(2))}
                ...
                <div className="d-flex align-items-end p-2">
                  <div className="border border-gray-300 border-dashed rounded w-100 ps-3 m-1">
                      <div className="fs-8 fw-bold text-gray-800 shrink-0">{intl.formatMessage({id: 'goal'})}</div>
                      <div className="fs-7 text-gray-800 fw-bolder shrink-0">#{count.goal_status_changed}</div>
                  </div>
                  <div className="border border-gray-300 border-dashed rounded w-100 ps-3 m-1">
                      <div className="fs-8 fw-bold text-gray-800 shrink-0">{intl.formatMessage({id: 'achieved'})}</div>
                      <div className="fs-7 text-gray-800 fw-bolder shrink-0">#{count.Achieved}</div>
                  </div>
                </div>                
              </div>
            </div>
