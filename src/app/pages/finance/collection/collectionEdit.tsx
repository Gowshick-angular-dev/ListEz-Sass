...
import { getCollectionById, getExpenses, getFeeConfirmations, getInvoiceById, getProformaInvoiceById, saveInvoice, updateCollection, updateInvoice } from '../core/_requests';
...
const CollectionEditForm: FC<props> = (props) => {
    const intl = useIntl();
    const {collectionsId, dropdowns} = props
    ...
    const [financeCollectionStatus, setFinanceInvoiceStatus] = useState<any[]>([]);
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, {setStatus, setSubmitting, resetForm}) => {
          ...
          try {
            ...
            const saveTaskData = await updateCollection(collectionsId, body);
            ...
            if(saveTaskData != null){
                setLoading(false);
                document.getElementById('kt_collection_details_close')?.click();
    ...
    const proformaInvoiceById = async (id:any) => {
        const response = await getCollectionById(id)
        setInvoiceDetails(response.output[0])

        formik.setFieldValue('invoice_date', Moment(response.output[0]?.invoice_date).format('YYYY-MM-DD') ?? '')
        ...
        formik.setFieldValue('total_invoiced_value', response.output[0]?.total_invoiced_value ?? '')
        formik.setFieldValue('total_collection_value', response.output[0]?.total_collection_value ?? '')
        formik.setFieldValue('total_due_value', response.output[0]?.total_due_value ?? '')
        formik.setFieldValue('bad_debts', response.output[0]?.bad_debts ?? '')
        formik.setFieldValue('notes', response.output[0]?.notes ?? '')
        formik.setFieldValue('status', response.output[0]?.collection_status ?? '')
    }
    ...
    const FinanceInvoiceStatusList =  async () => {
        const FinanceInvoiceStatusResponse = await getFinanceInvoiceStatus()
        setFinanceInvoiceStatus(FinanceInvoiceStatusResponse);
    }
    ...
    useEffect(() => {
        ...
        collectionsId && proformaInvoiceById(collectionsId);
    }, [collectionsId]);

    return(
        <div className='card shadow-none rounded-0 w-100'>
            <div className='card-header w-100' id='kt_proforma_header'>
                <h3 className='card-title fw-bolder text-dark'>{intl.formatMessage({id: 'edit_invoice'})}</h3>
                <div className='card-toolbar'>
                    <button
                    type='button'
                    className='btn btn-sm btn-icon btn-active-light-primary me-n5'
                    id='kt_collection_details_close'
                    >
