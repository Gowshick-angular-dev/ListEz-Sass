...
import { deleteProinvoice, getProformaInvoice } from '../core/_requests'
...
const ProformaInvoice = (props) => {
    ...
    const [proformaInvoiceId, setExpenseId] = useState('');
    ...
    const proformaInvoices = [
        ...
        // { field: 'created_names', title: 'Assigned To', cellStyle: {whiteSpace: 'nowrap'}},
        // { field: '', title: 'Mode', cellStyle: {whiteSpace: 'nowrap'} },
        { field: 'developer_name', title: 'Developer Name', cellStyle: {whiteSpace: 'nowrap'} },
        { field: 'date', render: rowData => Moment(rowData.date).format('DD-MMMM-YYYY') == "Invalid date" ? '' : Moment(rowData.date).format('DD-MMMM-YYYY'), title: 'Date', cellStyle: {whiteSpace: 'nowrap'} },
        ...
        { field: 'tax', title: 'Tax', render: rowData => parseInt(rowData.tax ?? 0) == 0 ? '' : parseInt(rowData.tax), cellStyle: {whiteSpace: 'nowrap'} },
        { field: 'total_amount', title: 'Total Amount', render: rowData => parseInt(rowData.total_amount ?? 0) == 0 ? '' : parseInt(rowData.total_amount), cellStyle: {whiteSpace: 'nowrap'}},
        // { field: 'contact_fullname', title: 'Contact Name', cellStyle: {whiteSpace: 'nowrap'}},
        ...
        <button className='btn btn-sm btn-icon' title='Edit' onClick={() => proformaInvoiceEdit(rowData.id)}>
        ...
        <button className='btn btn-sm btn-icon' title='Delete' onClick={() => proformaInvoiceDeletePop(rowData.id)}>
    ...
    const proformaInvoiceList = async () => {
        const response = await getProformaInvoice();
        setFinance(response.output);
    }

    const proformaInvoiceEdit = async (id) => {
        setExpenseId(id);
        document.getElementById('kt_proInvoice_details_toggle')?.click();

    }

    const proformaInvoiceDeletePop = (id) => {
        setExpenseId(id);
        document.getElementById('proformaInvoiceDeletePopup')?.click();
    }

    const proformaInvoiceDelete = async () => {
        const response = await deleteProinvoice(proformaInvoiceId)
        if(response != null) {
    ...
    return(<>
    <div
        id='kt_finance_proforma_invoice'
        className='bg-white'
        data-kt-drawer='true'
        data-kt-drawer-name='finance_proforma_invoice'
        data-kt-drawer-activate='true'
        data-kt-drawer-overlay='true'
        data-kt-drawer-width="{default:'100%', 'md': '700px'}"
        data-kt-drawer-direction='end'
        data-kt-drawer-toggle='#kt_proInvoice_details_toggle'
        data-kt-drawer-close='#kt_proInvoice_details_close'
      >
        <ProformaInvoiceEdit proformaInvoiceId={proformaInvoiceId} dropdowns={dropdowns}/>
    </div>
    <button className='d-none' id='proformaInvoiceReload' onClick={() => proformaInvoiceList()}>Reload</button>
    <button className='d-none' id='kt_proInvoice_details_toggle'>edit</button>
    <button data-bs-toggle='modal' data-bs-target={'#proformaInvoice_delete_confirm_popup'} className="d-none" id='proformaInvoiceDeletePopup'>Delete</button>
    <div className='modal fade' id={'proformaInvoice_delete_confirm_popup'} aria-hidden='true'>
            ...
            <div className='modal-content'>
                ...
                <div className='modal-body py-lg-10 px-lg-10'>
                    ...
                    <div className='d-flex align-items-center justify-content-end'>
                        ...
                        <button className='btn btn-sm btn_primary text-primary mt-3' data-bs-dismiss='modal' onClick={(e) => proformaInvoiceDelete()}>
        ...
        <div aria-atomic="true" aria-live="assertive" className="toast bg_primary p-0 text-white position-fixed end-0 bottom-0" id="ProInvoiceAdd">
            ...
            <div className="toast-body">
                <div>{intl.formatMessage({id: 'proforma_invoice_created_successfully'})}!</div>
            </div>
        </div>
        <div aria-atomic="true" aria-live="assertive" className="toast bg_primary p-0 text-white position-fixed end-0 bottom-0" id="ProInvoiceRemove">
            <div className="toast-header">
                <strong className="me-auto">{intl.formatMessage({id: 'success'})}</strong>
                <button aria-label="Close" className="btn-close btn-close-color-white" 
                    data-bs-dismiss="toast" type="button">
            ...
            <div className="toast-body">
                <div>{intl.formatMessage({id: 'proforma_invoice_removed_successfully'})}!</div>
            </div>
        </div>
        <div aria-atomic="true" aria-live="assertive" className="toast bg_primary p-0 text-white position-fixed end-0 bottom-0" id="ProInvoiceUpdate">
            <div className="toast-header">
                <strong className="me-auto">{intl.formatMessage({id: 'success'})}</strong>
                <button aria-label="Close" className="btn-close btn-close-color-white" 
                    data-bs-dismiss="toast" type="button">
            ...
            <div className="toast-body">
                <div>{intl.formatMessage({id: 'proforma_invoice_updated_successfully'})}!</div>
            </div>
        </div>
        <div aria-atomic="true" aria-live="assertive" className="toast bg_primary p-0 text-white position-fixed end-0 bottom-0" id="ProInvoiceErrMsg">
            <div className="toast-header">
        ...
        : <div className="text-center w-100">
            ...
            <p className='mt-3'>{intl.formatMessage({id: 'no_proForma_invoices_available'})}</p>
        </div>}
    </>
    )
}

export {ProformaInvoice}