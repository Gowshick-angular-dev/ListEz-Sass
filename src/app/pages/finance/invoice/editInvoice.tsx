...
import { getExpenses, getFeeConfirmations, getInvoiceById, getProformaInvoiceById, saveInvoice, updateInvoice } from '../core/_requests';
...
const InvoiceEditForm: FC<props> = (props) => {
    const intl = useIntl();
    const {invoicesId, dropdowns} = props
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, {setStatus, setSubmitting, resetForm}) => {
          ...
          try {
            ...
            const saveTaskData = await updateInvoice(invoicesId, body);
    
            if(saveTaskData != null){
                setLoading(false);
                document.getElementById('kt_Invoice_details_close')?.click();
    ...
    const proformaInvoiceById = async (id:any) => {
        ...
        formik.setFieldValue('brokerage_value', parseInt(response.output[0]?.brokerage_value ?? 0) == 0 ? '' : parseInt(response.output[0]?.total_amount))
        formik.setFieldValue('tax', parseInt(response.output[0]?.tax ?? 0) == 0 ? '' : parseInt(response.output[0]?.total_amount))
        ...
        formik.setFieldValue('status', response.output[0]?.status ?? '')
    ...
    useEffect(() => {
        ...
        invoicesId && proformaInvoiceById(invoicesId);
    }, [invoicesId]);
