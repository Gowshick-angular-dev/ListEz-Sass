...
import { deleteCashback, getCashback } from '../core/_requests'
import Moment from 'moment';
import { KTSVG } from '../../../../_metronic/helpers';
import { Toast } from 'bootstrap';
import { CashbackEditForm } from './cashbackEdit';
...
const Cashback = (props) => {
    ...
    const cashbacks = [
        ...
        { field: 'invoice_id', title: 'Invoice ID', cellStyle: {whiteSpace: 'nowrap'} },
        ...
        { field: 'transaction_amount', title: 'Total Amount', cellStyle: {whiteSpace: 'nowrap'}, render: row => parseInt(row.transaction_amount ?? 0) == 0 ? '' : parseInt(row.transaction_amount)},
        { field: 'due_date', title: 'Due Date', cellStyle: {whiteSpace: 'nowrap'} },
        { field: 'release_amount', title: 'Released Amount', cellStyle: {whiteSpace: 'nowrap'}, render: row => parseInt(row.release_amount ?? 0) == 0 ? '' : parseInt(row.release_amount) },
        { field: '', title: 'Due Amount', cellStyle: {whiteSpace: 'nowrap'}, render: row => parseInt(row.transaction_amount ?? 0) - parseInt(row.release_amount ?? 0)},        
        { field: 'id', render: rowData => <>
        <button className='btn btn-sm btn-icon' title='Edit' onClick={() => cashbackEdit(rowData.id)}>
        ...
        <button className='btn btn-sm btn-icon' title='Delete' onClick={() => cashbackDeletePop(rowData.id)}>
...
const cashbackList = async () => {
    ...
    const cashbackEdit = async (id) => {
        setCashbackId(id);
        document.getElementById('kt_Cashback_details_toggle')?.click();
    }

    const cashbackDeletePop = (id) => {
        setCashbackId(id);
        document.getElementById('cashbackDeletePopup')?.click();
    }

    const cashbackDelete = async () => {
        const response = await deleteCashback(cashbackId)
        if(response != null) {
    ...
    <div
        ...
        className='bg-white'
        data-kt-drawer='true'
        data-kt-drawer-name='finance_cashback'
        data-kt-drawer-activate='true'
        data-kt-drawer-overlay='true'
        data-kt-drawer-width="{default:'100%', 'md': '700px'}"
        data-kt-drawer-direction='end'
        data-kt-drawer-toggle='#kt_Cashback_details_toggle'
        data-kt-drawer-close='#kt_Cashback_details_close'
      >
        <CashbackEditForm cashbacksId={cashbackId} dropdowns={dropdowns} />
    </div>
    <button className='d-none' id='cashbackReload' onClick={() => cashbackList()}>Reload</button>
    <button className='d-none' id='kt_Cashback_details_toggle'>edit</button>
    <button data-bs-toggle='modal' data-bs-target={'#cashback_delete_confirm_popup'} className="d-none" id='cashbackDeletePopup'>Delete</button>
    <div className='modal fade' id={'cashback_delete_confirm_popup'} aria-hidden='true'>
            ...
            <div className='modal-content'>
                ...
                <div className='modal-body py-lg-10 px-lg-10'>
                    ...
                    <div className='d-flex align-items-center justify-content-end'>
                        ...
                        <button className='btn btn-sm btn_primary text-primary mt-3' data-bs-dismiss='modal' onClick={(e) => cashbackDelete()}>
        ...
        <div aria-atomic="true" aria-live="assertive" className="toast bg_primary p-0 text-white position-fixed end-0 bottom-0" id="ExpenseAdd">
            ...
            <div className="toast-body">
                <div>{intl.formatMessage({id: 'cashback_created_successfully'})}!</div>
            </div>
        </div>
        <div aria-atomic="true" aria-live="assertive" className="toast bg_primary p-0 text-white position-fixed end-0 bottom-0" id="ExpenseDelete">
            ...
            <div className="toast-body">
                <div>{intl.formatMessage({id: 'cashback_removed_successfully'})}!</div>
            </div>
        </div>
        <div aria-atomic="true" aria-live="assertive" className="toast bg_primary p-0 text-white position-fixed end-0 bottom-0" id="ExpenseUpdate">
            ...
            <div className="toast-body">
                <div>{intl.formatMessage({id: 'cashback_updated_successfully'})}!</div>
            </div>
        </div>
        ...
        : <div className="text-center w-100">
            ...
            <p className='mt-3'>{intl.formatMessage({id: 'no_cashback_available'})}</p>
        </div>}
    </>
