...
import { deleteExpense, getExpenses } from '../core/_requests'
...
const Expenses = (props) => {
    ...
    const expenses = [
        ...
        { field: 'expense_type_name', title: 'Expense Type', cellStyle: {whiteSpace: 'nowrap'}},
        { field: 'campaign_name', title: 'Caimpaign Name', cellStyle: {whiteSpace: 'nowrap'}},
        { field: 'property_name', title: 'Project', cellStyle: {whiteSpace: 'nowrap'}},
        { field: 'teamleader_name', title: 'Team Leader', cellStyle: {whiteSpace: 'nowrap'}},
        { field: 'client_fullname', title: 'Client Name', cellStyle: {whiteSpace: 'nowrap'}},
        { field: 'amount', title: 'Amount', cellStyle: {whiteSpace: 'nowrap'}, render: rowData => parseInt(rowData.amount) == 0 ? '' : parseInt(rowData.amount)},
        { field: 'expense_date', title: 'Expense Date', render: rowData => moment(rowData.expense_date).format('DD-MMMM-YYYY'), cellStyle: {whiteSpace: 'nowrap'}},
        { field: 'billable', title: 'Billable', cellStyle: {whiteSpace: 'nowrap'}, render: rowData => rowData.billable == 1 ? 'Yes' : 'No'},
        { field: 'transaction_id', title: 'Billable Transaction ID', cellStyle: {whiteSpace: 'nowrap'}},
        { field: 'id', render: rowData => <>
        <button className='btn btn-sm btn-icon' title='Edit' onClick={() => expenseEdit(rowData.id)}>
        ...
        <button className='btn btn-sm btn-icon' title='Delete' onClick={() => expenseDeletePop(rowData.id)}>
        ...
        </>, title: 'Actions', width: 180, cellStyle: {whiteSpace: 'nowrap'}},
    ];
    ...
    const expenseEdit = async (id) => {
        setExpenseEditId(id);
        document.getElementById('kt_expense_details_toggle')?.click();
    }

    const expenseDeletePop = (id) => {
        setExpenseId(id);
        document.getElementById('expenseDeletePopup')?.click();
    }
    ...
    return(<>
    <div
        id='kt_finance_details'
        className='bg-white'
        data-kt-drawer='true'
        data-kt-drawer-name='finance_details'
        data-kt-drawer-activate='true'
        data-kt-drawer-overlay='true'
        data-kt-drawer-width="{default:'100%', 'md': '700px'}"
        data-kt-drawer-direction='end'
        data-kt-drawer-toggle='#kt_expense_details_toggle'
        data-kt-drawer-close='#kt_expense_details_close'
      >
        <ExpenseEditForm expenseId={expenseEditId} dropdowns={dropdowns}/>
    </div>
    <button className='d-none' id='expenseReload' onClick={() => expenseList()}>Reload</button>
    <button className='d-none' id='kt_expense_details_toggle'>open</button>
    <button data-bs-toggle='modal' data-bs-target={'#expense_delete_confirm_popup'} className="d-none" id='expenseDeletePopup'>Delete</button>
    <div className='modal fade' id={'expense_delete_confirm_popup'} aria-hidden='true'>
            ...
            <div className='modal-content'>
                ...
                <div className='modal-body py-lg-10 px-lg-10'>
                    ...
                    <div className='d-flex align-items-center justify-content-end'>
                        ...
                        <button className='btn btn-sm btn_primary text-primary mt-3' data-bs-dismiss='modal' onClick={(e) => expenseDelete()}>
        ...
        <div aria-atomic="true" aria-live="assertive" className="toast bg_primary p-0 text-white position-fixed end-0 bottom-0" id="ExpenseAdd">
            ...
            <div className="toast-body">
                <div>{intl.formatMessage({id: 'expense_created_successfully'})}!</div>
            </div>
        </div>
        <div aria-atomic="true" aria-live="assertive" className="toast bg_primary p-0 text-white position-fixed end-0 bottom-0" id="ExpenseDelete">
            ...
            <div className="toast-body">
                <div>{intl.formatMessage({id: 'expense_removed_successfully'})}!</div>
            </div>
        </div>
        <div aria-atomic="true" aria-live="assertive" className="toast bg_primary p-0 text-white position-fixed end-0 bottom-0" id="ExpenseUpdate">
            ...
            <div className="toast-body">
                <div>{intl.formatMessage({id: 'expense_updated_successfully'})}!</div>
            </div>
        </div>
        ...
        : <div className="text-center w-100">
            ...
            <p className='mt-3'>{intl.formatMessage({id: 'no_expenses_available'})}</p>
        </div>}
    </>
    )
}
