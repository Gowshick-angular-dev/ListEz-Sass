...
const TaskSummaryReport: FC<Props> = (props) => {
  const intl = useIntl();
  const {
    ...
    const rangeSelectPie = (val:any) => {
      if(val == 7) {
        document.getElementById('taskSummaryDateRange')?.click();
      } else {
        setContactPie(val);
        var body = {
        'start_date': fromDate,
    ...
    return(
        <>
        <div className='modal fade' id='taskPieChartSummaryDate' data-bs-backdrop="static" data-bs-keyboard="false" aria-hidden='true'>
        ...
        <div className="mx-sm-1 mx-xl-2 h-100">        
            <div className="card-heade border-0 d-flex align-items-center justify-content-end px-5">        
              ...
              {list == 1 ? <>
                ...
                <button type='button' data-bs-toggle='modal' data-bs-target='#taskPieChartSummaryDate' className='d-none' id='taskSummaryDateRange'>{intl.formatMessage({id: 'custom_date'})}</button>                
                </div>                
            </div>             
            <div className="card-body">
              ...
              {contacts?.length > 0 ? 
                <TaskSummaryTable contacts={contacts} /> :
                <div className='w-100 h-100'> 
                    <div className='d-flex justify-content-center flex-column align-items-center h-100'>
