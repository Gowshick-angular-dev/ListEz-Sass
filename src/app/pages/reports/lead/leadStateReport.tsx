...
const LeadStateReport: FC<Props> = (props) => {
  const intl = useIntl();
  const {
    ...
    const CountSourceList =  async (userId:any, body:any, team:any, title:any) => {   
      ...
      setListTitle("Lead State wise Report " + `${userId == 0 ? ' for Team' : ' for user'}` + ` (${userName})  (${body.filter == 1 ? 'Today' : body.filter == 2 ? 'Yesterday' : body.filter == 3 ? 'Last Week' : body.filter == 4 ? 'Last Month' : body.filter == 5 ? 'This Month' : body.filter == 6 ? 'This Year' : body.start_date +''+ body.end_date}) ` + "Generated On " + moment().format('DD-MM-YYYY')); 
    }

    const rangeSelectPie = (val:any) => {
      if(val == 7) {
        document.getElementById('leadPieStateDateRange')?.click();
      } else {
        setContactPie(val);
        var body = {
        'start_date': fromDate,
    ...
    const handleClick = async (event:any, chartElements:any) => {
        if (chartElements && chartElements.length > 0) {
            ...
            const response = await getLeadReports(contactBody)
            setContacts(response.output); 
            setListTitle("Lead State wise Report - " + clickedLabel + `${reqId == 0 ? ' for Team' : ' for user'}` + ` (${userName})  (${contactPie == 1 ? 'Today' : contactPie == 2 ? 'Yesterday' : contactPie == 3 ? 'Last Week' : contactPie == 4 ? 'Last Month' : contactPie == 5 ? 'This Month' : contactPie == 6 ? 'This Year' : fromDate +''+ toDate}) ` + "Generated On " + moment().format('DD-MM-YYYY'));     
      };
    }
    ...
    return(
        <>
        <div className='modal fade' id='leadPieChartStateDate' data-bs-backdrop="static" data-bs-keyboard="false" aria-hidden='true'>
        ...
        <div className="mx-sm-1 mx-xl-2 h-100">        
            <div className="card-heade border-0 d-flex align-items-center justify-content-end px-5">        
              ...
              {list == 1 ? <>
                ...
                <button type='button' data-bs-toggle='modal' data-bs-target='#leadPieChartStateDate' className='d-none' id='leadPieStateDateRange'>{intl.formatMessage({id: 'custom_date'})}</button>                
                </div>                
            </div>             
