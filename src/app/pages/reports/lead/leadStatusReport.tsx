...
import { getContactReports, getLeadReports } from '../core/_request';
...
const LeadStatusReport: FC<Props> = (props) => {
  const intl = useIntl();
  const {
    setContacts, users, teams, setListTitle, contacts
  } = props
    ...
    const [title, setTitle] = useState('lead_status');
    ...
    const CountSourceList =  async (userId:any, body:any, team:any, title:any) => {   
      setLoading(true);
      //http://localhost:8080/orgReports/get_leads_report/5/2/0?filter_by=6&source=&lead_group=&property_type=&lead_status=&builder_name=&re_assign=&task_type=&city=
      let contactBody = {
        "module": 1,
        "userId": userId,
        "team": team,
      ...
      setTitle(title);
      setLoading(false);
      const response = await getLeadReports(contactBody)
      setContacts(response.output); 
      setListTitle("Contact Status Report " + `${userId == 0 ? ' for Team' : ' for user'}` + ` (${userName})  (${body.filter == 1 ? 'Today' : body.filter == 2 ? 'Yesterday' : body.filter == 3 ? 'Last Week' : body.filter == 4 ? 'Last Month' : body.filter == 5 ? 'This Month' : body.filter == 6 ? 'This Year' : body.start_date +''+ body.end_date}) ` + "Generated On " + moment().format('DD-MM-YYYY')); 
    }

    const rangeSelectPie = (val:any) => {
      if(val == 7) {
        document.getElementById('leadPieStatusDateRange')?.click();
      } else {
        setContactPie(val);
        var body = {
        'start_date': fromDate,
    ...
    const handleClick = async (event:any, chartElements:any) => {
        if (chartElements && chartElements.length > 0) {
            ...
            const response = await getLeadReports(contactBody)
            setContacts(response.output); 
            setListTitle("Contact Status Report - " + clickedLabel + `${reqId == 0 ? ' for Team' : ' for user'}` + ` (${userName})  (${contactPie == 1 ? 'Today' : contactPie == 2 ? 'Yesterday' : contactPie == 3 ? 'Last Week' : contactPie == 4 ? 'Last Month' : contactPie == 5 ? 'This Month' : contactPie == 6 ? 'This Year' : fromDate +''+ toDate}) ` + "Generated On " + moment().format('DD-MM-YYYY'));     
      };
    }
    ...
    return(
        <>
        <div className='modal fade' id='leadPieChartStatusDate' data-bs-backdrop="static" data-bs-keyboard="false" aria-hidden='true'>
        ...
        <div className="mx-sm-1 mx-xl-2 h-100">        
            <div className="card-heade border-0 d-flex align-items-center justify-content-end px-5">        
              ...
              {list == 1 ? <>
                ...
                <button type='button' data-bs-toggle='modal' data-bs-target='#leadPieChartStatusDate' className='d-none' id='leadPieStatusDateRange'>{intl.formatMessage({id: 'custom_date'})}</button>                
                </div>                
            </div>             
