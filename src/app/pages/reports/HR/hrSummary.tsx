...
import { getContactReports, getHRReports } from '../core/_request';
import { HrSummaryTable } from './hrSummaryTable';
...
const HRSummaryReport: FC<Props> = (props) => {
  const intl = useIntl();
  const {
    setContacts, contacts, users, teams, setListTitle
  } = props
    ...
    const [contactskwhiwhr, setContactskwhiwhr] = useState<any[]>([]);
    const [teamId, setTeamId] = useState(0);
    const [list, setList] = useState(1);

    console.log("eljrgwurghwrertkjg", contactskwhiwhr);
    ...
    const CountSourceList =  async (userId:any, body:any, team:any, title:any) => {   
      ...
      setContactskwhiwhr(response.output);
    }

    const rangeSelectPie = (val:any) => {
      if(val == 7) {
        document.getElementById('hrSummaryDateRange')?.click();
      } else {
        setContactPie(val);
        var body = {
        'start_date': fromDate,
    ...
    return(
        <>
        <div className='modal fade' id='hrPieChartSummaryDate' data-bs-backdrop="static" data-bs-keyboard="false" aria-hidden='true'>
        ...
        <div className="mx-sm-1 mx-xl-2 h-100">        
            <div className="card-heade border-0 d-flex align-items-center justify-content-end px-5">        
              ...
              {list == 1 ? <>
                ...
                <button type='button' data-bs-toggle='modal' data-bs-target='#hrPieChartSummaryDate' className='d-none' id='hrSummaryDateRange'>{intl.formatMessage({id: 'custom_date'})}</button>                
                </div>                
            </div>             
            <div className="card-body">
              ...
              {contactskwhiwhr?.length > 0 ? 
                <HrSummaryTable contacts={contactskwhiwhr} /> :
              <div className='w-100 h-100'> 
                <div className='d-flex justify-content-center flex-column align-items-center h-100'>
