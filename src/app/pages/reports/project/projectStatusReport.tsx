...
import { getContactReports, getProjectReports } from '../core/_request';
...
const ProjectStatusDoughnutReport: FC<Props> = (props) => {
  const intl = useIntl();
  const {
    ...
    const [title, setTitle] = useState('property_status');
    ...
    const graphXContact = count?.map((obj) => obj.count);
    const graphYContact = count?.map((obj) => obj.property_status);
    ...
    const CountSourceList =  async (userId:any, body:any, team:any, title:any) => {   
      ...
      const CountSourceResponse = await getMastersCount(title, 3, userId, team, body)
      ...
      setTitle(title); 
      setLoading(false);      
      const response = await getProjectReports(contactBody)
      setContacts(response.output); 
      setListTitle("Project Status Report " + ` (${userName})  (${body.filter == 1 ? 'Today' : body.filter == 2 ? 'Yesterday' : body.filter == 3 ? 'Last Week' : body.filter == 4 ? 'Last Month' : body.filter == 5 ? 'This Month' : body.filter == 6 ? 'This Year' : body.start_date +''+ body.end_date}) ` + "Generated On " + moment().format('DD-MM-YYYY')); 
    }

    const rangeSelectPie = (val:any) => {
      if(val == 7) {
        document.getElementById('projectPieStatusDateRange')?.click();
      } else {
        setContactPie(val);
        var body = {
        'start_date': fromDate,
    ...
    const handleClick = async (event:any, chartElements:any) => {
        if (chartElements && chartElements.length > 0) {
            ...
            const response = await getProjectReports(contactBody)
            setContacts(response.output); 
            setListTitle("Project Status Report - " + clickedLabel + ` (${userName})  (${contactPie == 1 ? 'Today' : contactPie == 2 ? 'Yesterday' : contactPie == 3 ? 'Last Week' : contactPie == 4 ? 'Last Month' : contactPie == 5 ? 'This Month' : contactPie == 6 ? 'This Year' : fromDate +''+ toDate}) ` + "Generated On " + moment().format('DD-MM-YYYY'));     
      };
    }
    ...
    return(
        <>
        <div className='modal fade' id='projectPieChartStatusDate' data-bs-backdrop="static" data-bs-keyboard="false" aria-hidden='true'>
            ...
            <div className="card-heade border-0 d-flex align-items-center justify-content-end px-5">        
              ...
              {list == 1 ? <>
                ...
                {!search
                    ...
                    <li className="dropdown-item border-0 ps-2 fs-7 py-2 cursor-pointer" key={i} onClick={() => userFilter(item.name_of_building, item.id)}>{item.name_of_building}</li>}
                    </>))
                    : filtered.length ? filtered.map((item, j) => (<>
                    {item.name_of_building &&
                      <li className="dropdown-item border-0 ps-2 fs-7 py-2 cursor-pointer" key={j} onClick={() => userFilter(item.name_of_building, item.id)}>{item.name_of_building}</li>}
                ...
                <button type='button' data-bs-toggle='modal' data-bs-target='#projectPieChartStatusDate' className='d-none' id='projectPieStatusDateRange'>{intl.formatMessage({id: 'custom_date'})}</button>                
                </div>                
            </div>             
...
export {ProjectStatusDoughnutReport}