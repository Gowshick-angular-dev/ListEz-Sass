...
import { deleteTemplateSMS, getAllTemplatesSMS, getTemplateSMS, updateTemplateSMS } from './core/_requests'
import { Toast } from 'bootstrap';
import * as Yup from 'yup'
import { useFormik } from 'formik'
import { saveTemplateSMS } from './core/_requests';
...
const SMS: FC = () => {
    ...
    const [allTemplatesSMS, setAllTemplatesSMS] = useState<any[]>([]);
    const [loading, setLoading] = useState(false); 
    const [dataBinded, setDataBinded] = useState(false); 
    const [templatesSMS, setTemplatesSMS] = useState<any[]>([]);
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, {setStatus, setSubmitting, resetForm}) => {
          ...
          try {
            ...
            console.log(body);
            if(!dataBinded){
                const saveTemplatSMSData = await saveTemplateSMS(body);
            
                if(saveTemplatSMSData != null){
                    setLoading(false);
                ...
                const updateTemplatSMSData = await updateTemplateSMS(selectedId, body);
            
                if(updateTemplatSMSData != null){
                    setLoading(false);
    ...
    const AllTemplatesSMSList =  async () => {
        const AllTemplatesSMSResponse = await getAllTemplatesSMS()
        setAllTemplatesSMS(AllTemplatesSMSResponse.output);
    }

    const SMSById = async (id:any) => {
        setSelectedId(id);
        const TemplatesSMSResponse = await getTemplateSMS(id)
        setTemplatesSMS(TemplatesSMSResponse.output);
        setDataBinded(true);
        formik.setFieldValue('title', TemplatesSMSResponse.output.title ?? '');
        formik.setFieldValue('subject', TemplatesSMSResponse.output.subject ?? '');
        formik.setFieldValue('share_with', TemplatesSMSResponse.output.share_with ?? '');
        formik.setFieldValue('module_id', TemplatesSMSResponse.output.module_id ?? '');
        formik.setFieldValue('body', TemplatesSMSResponse.output.body ?? '');
    }

    const deleteSMS = async (id:any) => {
        const deleteResponse = await deleteTemplateSMS(id);
    ...
    return(
        ...
        <div className='flex-column flex-lg-row-auto w-100 w-lg-300px w-xl-400px mb-10 mb-lg-0'>
            ...
            <div className='card-header pt-7' id='kt_chat_contacts_header'>
                <h3>{intl.formatMessage({id: 'sms_list'})}</h3>
            ...
            <div className='card-body pt-5' id='kt_chat_contacts_body'>
                ...
                >
                    ...
                    {!search
                        ? allTemplatesSMS.map((item) => (
                            <li className="nav-item w-100 mb-3" role="presentation" key={item.title}>
                            <div onClick={() => SMSById(item.id)} className='d-flex flex-stack py-4 nav-link active bg-light cursor-pointer' id="contact_status_tab" data-bs-toggle="pill" data-bs-target="#contact_status" role="tab" aria-controls="contact_status" aria-selected="true">
        ...
        <div className='flex-lg-row-fluid ms-lg-7 ms-xl-10'>
            <div className="card">
                <div className='card-header pt-7' id='kt_chat_contacts_header'>
                    <h3>{intl.formatMessage({id: 'sms_field'})}</h3>
                </div>
                                ...
                                <div className="card bs_1">
                                    <form noValidate onSubmit={formik.handleSubmit} >
                                                    ...
                                                    <div className="row">
                                                        ...
                                                        <div className="col-12">                                        
                                                            <div className="form-group mb-4">
                                                                ...
                                                                <div className="input-group">
                                                                    <textarea style={{height: '200px'}} className="form-control" placeholder="Encryption" {...formik.getFieldProps('body')}/> 
                                                                </div>
                                                                {formik.touched.body && formik.errors.body && (
                                                            <div className='fv-plugins-message-container'>
                                                                <div className='fv-help-block'>
                                                                    <span role='alert' className='text-danger'>{formik.errors.body}</span>
                                                                </div>
                                                            </div>
                                                            )}
                                                            </div>
                                        ...
                                        <div className='card-footer py-5 text-center' id='kt_task_footer'>
                                            ...
                                            data-bs-target={'#delete_confirm_popupsms'+selectedId}
                                            >
        ...
        {/* confirm modal */}
        <div className='modal fade' id={'delete_confirm_popupsms'+selectedId} aria-hidden='true'>
                ...
                <div className='modal-content'>
                    ...
                    <div className='modal-body py-lg-10 px-lg-10'>
                        ...
                        <div className='d-flex align-items-center justify-content-end'>
                            ...
                            <button className='btn btn-sm btn_primary text-primary mt-3' data-bs-dismiss='modal' onClick={(e) => deleteSMS(selectedId)}>
        ...
        <div aria-atomic="true" aria-live="assertive" className="toast bg_primary toast_position text-white position-fixed end-0 bottom-0 m-3" id="myToastAdd">
            ...
            <div className="toast-body">
                {intl.formatMessage({id: 'sms_saved_successfully'})}!
            </div>
        </div>
        <div aria-atomic="true" aria-live="assertive" className="toast bg_primary toast_position text-white position-fixed end-0 bottom-0 m-3" id="myToastUpdate">
            ...
            <div className="toast-body">
                {intl.formatMessage({id: 'sms_updated_successfully'})}!
            </div>
        </div>
        <div aria-atomic="true" aria-live="assertive" className="toast bg_primary toast_position text-white position-fixed end-0 bottom-0 m-3" id="myToastDelete">
            ...
            <div className="toast-body">
                {intl.formatMessage({id: 'sms_deleted_successfully'})}!
            </div>
        </div>
    </div>  
)
