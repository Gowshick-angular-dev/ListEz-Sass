...
import { getOrgDropList, updateOrganozationCompany } from './core/_requests';
...
const OrganizationEdit:  FC<Props> = (props) => {
    const intl = useIntl();

    const {orgId, setList} = props    
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, {setStatus, setSubmitting, resetForm}) => {
          ...
          try {
            ...
            const saveTemplateMailData = await updateOrganozationCompany(orgId.id, formData);
            if(saveTemplateMailData.status == 200){
                document.getElementById('orgReload')?.click();
                // setList(saveTemplateMailData.output.slice(0, -1));
                document.getElementById('kt_organization_edit_close')?.click();
                imgRemove();
                setLoading(false);
      ...
      const handleImagePreview = (e:any) => {
        if(e.target.files[0].size > 2097152){
          ...
          if(isValidFileUploaded(file)){
            ...
            (document.getElementById('logo_image_edit') as HTMLInputElement).value = '';
      ...
      const handleWatermarkPreview = (e:any) => {
        if(e.target.files[0].size > 2097152){
          ...
          if(isValidFileUploaded(file)){
            ...
            (document.getElementById('watermark_image_edit') as HTMLInputElement).value = '';
    ...
    const loadOrg = async (data:any) => {
        if(data) {
            formik.setFieldValue('organization_name', data.organization_name ?? "");
            formik.setFieldValue('phone_number', data.phone_number ?? "");
            formik.setFieldValue('gst_number', data.gst_number ?? "");
            formik.setFieldValue('cin_number', data.cin_number ?? "");
            formik.setFieldValue('tan_number', data.tan_number ?? "");
            formik.setFieldValue('pan_number', data.pan_number ?? "");
            formik.setFieldValue('registered_address', data.address ?? "");
            ...
            formik.setFieldValue('facebook_url', data.facebook_url ?? "");
            formik.setFieldValue('linkindin_url', data.linkindin_url ?? "");
            formik.setFieldValue('twitter_url', data.twitter_url ?? "");
            formik.setFieldValue('instagram_url', data.instagram_url ?? "");
            formik.setFieldValue('youtube_url', data.youtube_url ?? "");            
            formik.setFieldValue('first_name', data.user_name ?? "");
            formik.setFieldValue('email', data.user[0]?.email ?? "");
            setApproval(data.approval == 1 ? true : false);
        }
    }
    ...
    return(
        <div className='card shadow-none w-100'>
            <div className='card-header w-100' id='kt_task_header'>
                <h3 className='card-title fw-bolder text-dark'>{orgId.id == 1 ? intl.formatMessage({id: 'edit_admin'}) : intl.formatMessage({id: 'edit_organization'})}</h3>

                <div className='card-toolbar'>
                    <button
                    type='button'
                    className='btn btn-sm btn-icon btn-active-light-primary me-n5'
                    id='kt_organization_edit_close'
                    >
                                ...
                                <div className="row">
                                    <div className="col-md-6 p-3">
                                        ...
                                        <a onClick={(e) => imgRemove()} className="icon position-absolute px-1 top-0 end-0 rounded bg-gray border-0"><span className="svg-icon svg-icon-muted"><svg width="" height="30" viewBox="3 3 18 18" fill="none" xmlns="http://www.w3.org/2000/svg"><rect opacity="0.3" x="2" y="2" width="20" height="20" rx="10" fill="black"></rect><rect x="7" y="15.3137" width="12" height="2" rx="1" transform="rotate(-45 7 15.3137)" fill="white"></rect><rect x="8.41422" y="7" width="12" height="2" rx="1" transform="rotate(45 8.41422 7)" fill="white"></rect></svg></span></a></div></>}
                                            <span className="btn btn-file d-flex flex-md-row p-2 p-md-4 w-75 my-3">
                                                ...
                                                <input type="file" accept="image/*" id='logo_image_edit' onChange={handleImagePreview}/>
                                            </span>
                                    ...
                                    <div className="col-md-6 p-3">
                                        <div className='d-flex justify-content-center flex-wrap bg-secondary p-5 br_10'>
                                            ...
                                            <span className="btn btn-file d-flex flex-md-row p-2 p-md-4 w-75 my-3">
                                                <i className="fa fa-upload my-2" aria-hidden="true"></i>{intl.formatMessage({id: 'upload_atermark'})}
                                                <input type="file" accept="image/*" id='watermark_image_edit' onChange={handleWatermarkPreview}/>
                                            </span>
                                            <div className='fv-plugins-message-container'>
                                                <div className='fv-help-block'>
                                                    <span role='alert' className='fs-10px text-danger'>Note: use jpg, jpeg, png only and Size must below 2 MB!</span>
                                    ...
                                    <div className="col-6">
                                        <div className="form-group mb-4">
                                            ...
                                            <select className="form-select btn-sm text-start" {...formik.getFieldProps('currency')}>
                                                <option selected value="">Select</option>
                                                {dropList.currency?.map((data: any, i: React.Key | null | undefined) => {
                                                    return(
