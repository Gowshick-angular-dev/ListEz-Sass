...
import { updateTaskStatus } from './core/_requests'
...
function ControlledBoardTask({boardData, reload}) {
    ...
    function handleCardMove(_card, source, destination) {
        ...
        if(source.fromColumnId != destination.toColumnId) {
            document.getElementById("task_status_pop_kanban")?.click();
        }
        setMovedDetails({
            "source": source,
    ...
    const formikNotes = useFormik({
        ...
        onSubmit: async (values, {setStatus, setSubmitting, resetForm}) => {
          try {   
                let body = {
                    "task_status": movedDetails.destination?.toColumnId,
                    "reply": values.reply
                }
            const updateStatus = await updateTaskStatus(movedDetails._card?.id, body);
    ...
    useEffect(() => {
        ...
        if(data?.length > 0) {
            const kanbanBoard = data?.reduce((group, product) => {
                const { task_status_name } = product;
                group[task_status_name] = group[task_status_name] ?? [];
                group[task_status_name].push(product);
                return group;
            }, {}); 
            const kanbanData = [];              
            for (const key in kanbanBoard) {
                if(kanbanBoard[key][0].task_status) {
                let col = {
                id: kanbanBoard[key][0].task_status,
                title: key,
                backgroundColor: "#fff",
                cards: kanbanBoard[key]?.map((t) => {
                    let jhfgwe = {
                        ...
                        image: t.contact_profile_image,
                        title: t.task_type_name,
                        property: t.property_name,
                        time: t.task_time_at,
                        created: t.created_at,
                        priority: t.priority_name,
                        mobile: t.contact_mobile,
                        description: t.agenda
                    }
                    return jhfgwe;
                })
                };
    ...
    return (<>
    <a className="d-none" href="#" data-bs-toggle='modal' id='task_status_pop_kanban' data-bs-target={'#contact_status_change_kanban'}></a>
        ...
        <div>
            ...
            <Board renderCard={(cards, { removeCard, dragging }) => (
                <div className="card kanban_card my-2 br_10 overflow-hidden" dragging={dragging}>
                    <div className={cards.priority == "High Priority" ? "px-3 py-2 bg-danger" : cards.priority == "Medium Priority" ? "px-3 py-2 bg-warning" : cards.priority == "Low Priority" ? "px-3 py-2 bg-primary" : "px-3 py-2 bg_secondary"}>                
                        <div className="d-flex justify-content-between w-100">
                            <div className="flex-grow-1 ms-2">
                                <p className="mb-0 fw-bolder">{cards.title ?? "Others"}</p>
                    ...
                    <div className="card-body p-3">
                        <div className="row">
                            ...
                            {/* <div className="col-sm-6 col-6 mb-3" title={cards.contact ?? '--'}>
                                <div className="task_content_single">
                                    <div className="d-flex align-items-start single_item">
                                        <img src={toAbsoluteUrl('/media/custom/project.svg')} alt="" className="me-2"/>
                                        <div className="d-flex flex-column">
                                            <small className="text_light">{intl.formatMessage({id: 'name'})}</small>
                                            <p className="mb-0 fw-500">{cards.contact ?? '--'}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="col-sm-6 col-6 mb-3" title={cards.mobile ?? '--'}>
                                <div className="task_content_single">
                                    <div className="d-flex align-items-start single_item">
                                        <img src={toAbsoluteUrl('/media/custom/lead/assign_9.svg')} alt="" className="me-2"/>
                                        <div className="d-flex flex-column">
                                            <small className="text_light">{intl.formatMessage({id: 'phone'})}</small>
                                            <p className="mb-0 fw-500">{cards.mobile ?? '--'}</p>
                                        </div>
                                    </div>
                                </div>
                            </div> */}
                            <div className="col-12">
                                <div className="task_content_single">
                                    <div className="d-flex align-items-start single_item">
                                        ...
                                        <div className="d-flex flex-column">
                                            <small className="text_light mb-0">{intl.formatMessage({id: 'task_description'})}</small>
                                            {cards.id != expandId ? <>
                                            <p className="mb-0 fw-500 fs-9">{cards.description ? cards.description?.slice(0, 100)+(cards.description?.length > 100 ? '...' : '') : '--'}</p>
                                            {cards.description?.length > 100 ? <a onClick={() => setExpandId(cards.id)} className="text-primary cursor-pointer">Read more...</a> : ''}
                                            </> : <>
                                            <p className="mb-0 fw-500 fs-9">{cards.description ?? '--'}</p>
